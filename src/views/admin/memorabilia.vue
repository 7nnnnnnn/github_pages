<template>
  <div>
    <el-input v-model="searchText" placeholder="请输入搜索编号" clearable @clear="clearSearch" @keyup.enter="performSearch"
  ...
  <div>
    <el-table :data="paginatedData" :header-cell-style="{ background: '#eef1f6', color: '#606266' }" border>
      ...
      <el-table-column prop="happenTime" label="大事件发生时间"></el-table-column>
...
export default {
  ...
  methods: {
      ...
      this.apiList = [],
        api.getOneData(id)
          .then(response => {
            ...
            const classrooms = response.data.memorabilia;
    ...
    addAPI() {
      ...
      const now = new Date(a); //创建一个指定的日期对象
      const year = now.getFullYear(); //年份
      const month = now.getMonth() + 1; //月份（0-11）
      const date = now.getDate(); //天数（1到31）
      const hour = now.getHours(); //小时数（0到23）
      const minute = now.getMinutes(); //分钟数（0到59）
      const second = now.getSeconds(); //秒数（0到59）
      ...
      const newAPI = {
          ...
          "createTime":"",
          "content":"",
          "updateTime":year + "-" + month + "-" + date + " " + hour + ":" + minute + ":" + second,
          "happenTime":""
      };
